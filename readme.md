...
- 可以使用交叉验证优化超参数，如下所示（在我们的示例中，我们尝试预测k）：
1. 将数据集拆分为f份。
2. 给定预测的超参数：
    - 选用用f-1份数据训练算法，并用剩余的那一份去测试它。然后不断地重复这个过程直到完成f次训练。
3. 选择提供最佳训练结果的超参数
- 线性SVM（Linear Support Vector Machine，支持向量机）分类器也是解决图像分类问题的一种选择，但维数的局限性使模型在某一点上不再收敛。
- LR（Logistic regression逻辑回归）也可能是图像分类问题的一种解决方案，但图像分类问题是非线性的，因此效果也十分地不好！
